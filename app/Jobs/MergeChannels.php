...
class MergeChannels implements ShouldQueue
{
    ...
    protected function processGroupedChannels(iterable $groupedChannels, array $playlistPriority): array
    {
        ...
        foreach ($groupedChannels as $group) {
            ...
            $failoverChannels = $this->sortFailoverChannelsByScore(
                $this->filterFailoverCandidates($group->where('id', '!=', $master->id), $master),
                $playlistPriority,
            );
    ...
    protected function processFallbackNameMerges(
        ...
        $channels = $channelsQuery
            ...
            ->select(['id', 'user_id', 'playlist_id', 'group_id', 'title', 'title_custom',
                'name', 'name_custom', 'stream_id', 'stream_id_custom', 'url', 'url_custom', 'sort',
    ...
    protected function processRegexMerges(array $playlistIds, array $playlistPriority): array
    {
        ...
        foreach ($validPatterns as $i => $pattern) {
            ...
            $sorted = $this->sortFailoverChannelsByScore(
                $this->filterFailoverCandidates($matches->where('id', '!=', $master->id), $master),
                $playlistPriority,
            );
     ...
     *
     * Also drops candidates whose resolved stream URL is identical to the
    ...
    /**
     * Resolve the effective playback URL for a channel (custom override wins),
     * matching the pattern used throughout the Channel model, or null when
     * neither is set so an empty URL never accidentally matches another.
     */
    protected function resolveEffectiveUrl(Channel $channel): ?string
    {
