        ...
        @foreach ($phases as $phase)
            ...
            <li class="relative {{ $isLast ? '' : 'pb-6' }}">
                ...
                <div class="relative flex gap-4">
                    {{-- Status circle --}}
                    @if ($status === 'completed')
                        <div class="bg-success-100 text-success-600 dark:bg-success-900 dark:text-success-400 relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                            </svg>
                        </div>
                    @elseif ($status === 'running')
                        <div class="bg-warning-100 text-warning-600 dark:bg-warning-900 dark:text-warning-400 relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
                    ...
                    @elseif ($status === 'failed')
                        <div class="bg-danger-100 text-danger-600 dark:bg-danger-900 dark:text-danger-400 relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="flex min-w-0 flex-1 items-center justify-between gap-4 pt-0.5">
                        <div class="min-w-0">
                            ...
                            @elseif ($status === 'running')
                                <p class="text-warning-600 dark:text-warning-400 text-xs leading-5">Running…</p>
                            @endif
                        </div>
        ...
        @if ($showPostProcessingPlaceholder)
            ...
            <li class="relative">
                <div class="relative flex gap-4">
                    ...
                    <div class="flex min-w-0 flex-1 items-center justify-between gap-4 pt-0.5">
                        <div class="min-w-0">
                            <p class="text-sm leading-6 font-medium text-gray-400 dark:text-gray-500">
                                Post-processing
                            </p>
                            <p class="text-xs leading-5 text-gray-500 italic dark:text-gray-400">Resolving phases…</p>
                        </div>
                        <span class="inline-flex shrink-0 items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                            Pending
                        </span>
                    </div>
                </div>
