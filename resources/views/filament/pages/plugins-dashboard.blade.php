                ...
                <x-filament::card>
                    <div class="flex items-start gap-4">
                        <div @class([
                            ...
                            <x-dynamic-component
                                :component="$card['icon']"
                                @class(['h-5 w-5', 'text-green-900 dark:text-green-100' => $card['color'] === 'green', 'text-amber-900 dark:text-amber-100' => $card['color'] === 'amber', 'text-red-900 dark:text-red-100' => $card['color'] === 'red', 'text-blue-900 dark:text-blue-100' => ! in_array($card['color'], ['green', 'amber', 'red'])])
                            />
                        </div>
                        <div class="flex min-w-0 items-center gap-1">
                            <p class="mr-1 text-sm font-medium text-gray-600 dark:text-gray-400">
                                {{ $card['label'] }}
                            </p>
        ...
        <div class="grid grid-cols-1 gap-4">
            <x-filament::card>
                ...
                @if ($attentionPlugins->isEmpty())
                    <div class="mt-4 rounded-xl border border-dashed border-green-300 bg-green-50 p-4 text-sm text-green-700 dark:border-green-900 dark:bg-green-950/40 dark:text-green-300">
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div class="min-w-0">
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                            {{ __('Trust') }}: {{ str($plugin->trust_state ?: 'pending_review')->replace('_', ' ')->headline() }} · {{ __('Files') }}: {{ str($plugin->integrity_status ?: 'unknown')->replace('_', ' ')->headline() }} · {{ __('Status') }}: {{ str($plugin->installation_status ?: 'installed')->replace('_', ' ')->headline() }}
                                        </p>
                                    </div>
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            <x-filament::badge
                                                :color="$this->installStatusColor($review->status)"
                                                size="sm"
                                            >
                                        ...
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            {{ __('Scan') }}: {{ str($review->scan_status ?: 'pending')->replace('_', ' ')->headline() }} · {{ optional($review->created_at)->diffForHumans() }}
                                        </p>
                                    </div>
