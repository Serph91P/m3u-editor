<div class="flex flex-col gap-y-1">
    ...
    @elseif ($isPostProcessing)
        <div class="flex items-center gap-2">
            <svg
                class="h-3.5 w-3.5 shrink-0 animate-spin text-indigo-500 dark:text-indigo-400"
    ...
    @else
        <x-filament::badge size="sm" :color="$status?->getColor() ?? 'gray'"> {{ $label }} </x-filament::badge>
    @endif

    @if ($postProcessingStep)
        <span class="text-xs leading-tight text-indigo-600 dark:text-indigo-400">{{ $postProcessingStep }}</span>
    @endif
</div>
