...
>
                ...
                <div class="py-3 first:pt-0 last:pb-0">
                    ...
                    <div class="mt-1 flex items-center gap-3 text-xs text-gray-500 dark:text-gray-500">
                        ...
                        @if ($item['protocol'] ?? null)
                            <span class="tracking-wide text-gray-400 uppercase dark:text-gray-600">{{ $item['protocol'] === 'usenet' ? 'NZB' : ucfirst($item['protocol']) }}</span>
                        @endif
                    ...
                    @if (($item['source'] ?? null) === 'media_request' && ($item['requested_by'] ?? null))
                        <p class="mt-0.5 truncate text-xs text-gray-400 dark:text-gray-600">
                            {{ __('Requested by: :name', ['name' => $item['requested_by']]) }}
                        </p>
                    @elseif ($item['indexer'] ?? null)
                        <p class="mt-0.5 truncate text-xs text-gray-400 dark:text-gray-600">{{ $item['indexer'] }}</p>
                    @endif
    ...
    @elseif (! $queueGroup['error'])
        <p class="text-center text-xs text-gray-400 dark:text-gray-600">{{ __('Queue is empty') }}</p>
    @endif
</x-filament::section>
