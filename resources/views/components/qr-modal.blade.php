<x-filament::modal icon="heroicon-o-qr-code" alignment="center">
    <x-slot name="trigger">
        <x-filament::button icon="heroicon-m-qr-code" color="gray"> {{ $label ?? 'QR Code' }} </x-filament::button>
    </x-slot>

    <x-slot name="heading">{{ $title }}</x-slot>

    <div class="relative flex w-auto flex-col items-center gap-2" wire:ignore>
        <p>{{ $body }}</p>
